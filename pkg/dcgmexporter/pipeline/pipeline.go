...
func (m *MetricsPipeline) run() (string, error) {
	var metrics map[common.Counter][]collector.Metric
	var err error
	...
	if !common.IsNil(m.gpuCollector){
		...
		for _, transform := range m.transformations {
			err := transform.Process(metrics, m.gpuCollector.GetSysinfo())
			if err != nil {
...
func FormatMetrics(t *template.Template, groupedMetrics collector.MetricsByCounter) (string, error) {
	// Format metrics
