...
func (r *Registry) Gather() (collector.MetricsByCounter, error) {
	r.mtx.Lock()
	defer r.mtx.Unlock()
	...
	for _, c := range r.collectors {
		...
		g.Go(func() error {
			...
			for counter, metricVals := range metrics {
				val, _ := sm.LoadOrStore(counter, []collector.Metric{})
	...
	output := collector.MetricsByCounter{}

	sm.Range(func(key, value interface{}) bool {
		output[key.(common.Counter)] = value.([]collector.Metric)
