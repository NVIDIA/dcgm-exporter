...
func GetCounterSet(c *common.Config) (*common.CounterSet, error) {
	var (
		err     error
		records [][]string
	)

	res := new(common.CounterSet)
...
func ExtractCounters(records [][]string, c *common.Config) (*common.CounterSet, error) {
	res := common.CounterSet{}
	...
	for i, record := range records {
		...
		if !ok && !oldOk {
			...
			expField, err := metrics.IdentifyMetricType(record[0])
			if err != nil {
				return nil, fmt.Errorf("could not find DCGM field; err: %w", err)
			} else if expField != metrics.DCGMFIUnknown {
				res.ExporterCounters = append(res.ExporterCounters,
					common.Counter{dcgm.Short(expField), record[0], record[1], record[2]})
				continue
			}
		}
		...
		if !useOld {
			...
			res.DCGMCounters = append(res.DCGMCounters, common.Counter{fieldID, record[0], record[1], record[2]})
		} else {
			...
			res.DCGMCounters = append(res.DCGMCounters, common.Counter{oldFieldID, record[0], record[1], record[2]})
		}
	}
...
func fieldIsSupported(fieldID uint, c *common.Config) bool {
...
func readConfigMap(kubeClient kubernetes.Interface, c *common.Config) ([][]string, error) {
