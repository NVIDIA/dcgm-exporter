...
- `nvidia/dcgm-exporter:4.5.3-4.8.2-distroless`
- `nvidia/dcgm-exporter:4.5.3-4.8.2-ubuntu22.04`
- `nvidia/dcgm-exporter:4.5.3-4.8.2-ubi9`
...
FULL_VERSION=4.5.3-5.0.0 make docker-test-ubuntu
```
...
IMAGE_UBUNTU=nvcr.io/nvidia/k8s/dcgm-exporter:4.5.3-4.8.2-ubuntu22.04 \
make docker-test-ubuntu
...
| `FULL_VERSION` | `4.5.3-4.8.2` | Combined DCGM and exporter version (updated by root Makefile) |
...
FULL_VERSION=4.5.3-5.0.0-rc1 make docker-test
...
IMAGE_UBUNTU=nvidia/dcgm-exporter:4.5.3-4.8.2-ubuntu22.04 \
IMAGE_DISTROLESS=nvidia/dcgm-exporter:4.5.3-5.0.0-distroless \
IMAGE_UBI="" \
make docker-test
```
...
make update-version \
  ...
  NEW_DCGM_VERSION=4.5.3 \
  OLD_EXPORTER_VERSION=4.5.3 \
  NEW_EXPORTER_VERSION=4.8.2

# This will update FULL_VERSION in tests/docker/Makefile from 4.3.0-4.5.3 to 4.5.3-4.8.2
```
...
docker run --rm --gpus all nvidia/cuda:13.2.1-base nvidia-smi

# Verify DCGM access
docker run --rm --gpus all --cap-add SYS_ADMIN nvcr.io/nvidia/k8s/dcgm-exporter:4.5.3-4.8.2-distroless
```
