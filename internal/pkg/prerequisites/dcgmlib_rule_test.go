...
func Test_dcgmLibExistsRule_Validate(t *testing.T) {
	...
	testCases := []testCase{
		{
			...
			Cache generated by: ldconfig (Ubuntu GLIBC 2.35-0ubuntu3.7) stable release version 2.35`
				...
				mockExec.EXPECT().CommandContext(gomock.Any(), gomock.Eq(ldconfigPath), gomock.Eq(ldconfigParam)).AnyTimes().Return(cmd)
			},
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			// Pretend /sbin/ldconfig.real exists so the rule picks it without
			// falling back to PATH lookup — keeps tests deterministic on
			// hosts that do not ship /sbin/ldconfig.real (e.g. NixOS).
			osMock := mockos.NewMockOS(ctrl)
			fi := mockos.NewMockFileInfo(ctrl)
			osMock.EXPECT().Stat(ldconfigPath).Return(fi, nil).AnyTimes()
			os = osMock
			...
			err := dcgmLibExistsRule{}.Validate(context.Background())
			tc.AssertErr(err)
		})
	}
}
