	...
	testCases := []testCase{
		{
			...
			ELFMockExpectations: func(c *gomock.Controller, mockELF *mockelf.MockELF) {
				...
				mockELF.EXPECT().Open(gomock.Eq("/lib/x86_64-linux-gnu/libdcgm.so")).AnyTimes().Return(libdcgm, nil)
			},
			AssertErr: func(err error) {
		...
		},
		{
			...
			AssertErr: func(err error) {
				require.Error(t, err)
				require.ErrorContains(t, err, "the libdcgm.so library was not found. Install Data Center GPU Manager (DCGM).")
			},
		},
		{
		...
		},
		{
			...
			AssertErr: func(err error) {
				...
				require.ErrorContains(t, err,
					"the libdcgm.so library architecture mismatch with the system; wanted: EM_X86_64, received: EM_AARCH64")
			},
		},
		{
			...
			ELFMockExpectations: func(c *gomock.Controller, mockELF *mockelf.MockELF) {
				...
				mockELF.EXPECT().Open(gomock.Eq("/lib/x86_64-linux-gnu/libdcgm.so")).AnyTimes().Return(nil, errors.New("boom!"))
			},
			AssertErr: func(err error) {
				require.Error(t, err)
			},
