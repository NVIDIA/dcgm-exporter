...
func TestValidate(t *testing.T) {
	...
	tests := []testCase{
		{
			...
			OSMockExpectations: func(ctrl *gomock.Controller, mo *mockos.MockOS) {
				...
				// First Stat picks /sbin/ldconfig.real; second confirms it exists
				// so we do not fall back to PATH lookup.
				mo.EXPECT().Stat("/sbin/ldconfig.real").Return(mfi, nil).Times(2)
			},
		...
		},
		{
			...
			OSMockExpectations: func(ctrl *gomock.Controller, mo *mockos.MockOS) {
				...
				// Second Stat confirms /sbin/ldconfig exists.
				mo.EXPECT().Stat("/sbin/ldconfig").Return(mfi, nil)
			},
	...
	for _, tc := range tests {
		...
		executor.EXPECT().CommandContext(gomock.Any(), gomock.Eq(tc.LDConfigPath), gomock.Eq(ldconfigParam)).AnyTimes().Return(cmd)
		...
		err := Validate(context.Background())
